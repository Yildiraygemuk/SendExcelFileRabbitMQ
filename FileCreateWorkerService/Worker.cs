...
namespace FileCreateWorkerService
{
    public class Worker : BackgroundService
    {
        ...
        private async Task Consumer_Received(object sender, BasicDeliverEventArgs @event)
        {
            ...
            var ds = new DataSet();
            ds.Tables.Add(GetTable("books"));
            wb.Worksheets.Add(ds);
            wb.SaveAs(ms);
            MultipartFormDataContent multipartFormDataContent = new MultipartFormDataContent();
            multipartFormDataContent.Add(new ByteArrayContent(ms.ToArray()), "file", Guid.NewGuid().ToString() + ".xlsx");
            var baseUrl = "https://localhost:44341/api/books/Upload";
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                {
                    _logger.LogInformation($"Excel mail has been sent to file {createExcelMessage.UserMail} address File.");
        ...
        private DataTable GetTable(string tableName)
        {
            ...
            books.ForEach(x =>
            {
                table.Rows.Add(x.Id, x.BookName, x.Description, x.CategoryId);
            });
            return table;
        }
    }
